...
auto_adjust_enemy_died( amount, attacker, type, point )
{
	...
	*/
	...
	if ( player_attacker(attacker) ) {
		foreach ( player in level.players ) {
		/*
			player.hud_scorePopup.label = "+";
			player.hud_scorePopup.alpha = 1;
			player.hud_scorePopup setValue(100);
			player thread scorePopup( 10, 0, (1,1,1), 0 );
			wait ( 1.0 );
			player.hud_scorePopup fadeOverTime( 0.75 );
			player.hud_scorePopup.alpha = 0;
		*/
			//thread testScore(player, (1,1,1));
		}
	}
	//prof_end( "auto_adjust_enemy_died" );
}

testScore(player, hudColor) {
	player.hud_scorePopup.color = hudColor;
	player.hud_scorePopup.glowColor = hudColor;
	player.hud_scorePopup.label = "+";
	player.hud_scorePopup.alpha = 1;
	player.hud_scorePopup setValue(100);
	wait ( 1.0 );
